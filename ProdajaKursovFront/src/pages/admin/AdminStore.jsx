...
export default function AdminStore() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const list = await api.courses.storeCards.list()
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const list = await api.courses.listSimple()
        const normalized = Array.isArray(list) ? list.map(c => ({
          id: String(c.id),
          title: c.title || '',
          slug: c.slug || '',
  ...
  const submit = async (e) => {
    ...
    const saved = editingId
      ? await api.courses.storeCards.update(editingId, payload)
      : await api.courses.storeCards.create(payload)
    const normalized = {
      id: saved.id,
      title: saved.title || '',
  ...
  const remove = async (id) => {
    await api.courses.storeCards.remove(id)
