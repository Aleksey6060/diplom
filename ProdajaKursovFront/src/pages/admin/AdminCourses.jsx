...
function AdminCoursesBackend() {
  ...
  const loadNode = async (type, id) => {
    ...
    try {
      let data
      if (type === 'course') data = await api.courses.courseContents(id)
      else if (type === 'semester') data = await api.courses.semesterContents(id)
      else if (type === 'subject') data = await api.courses.subjectContents(id)
      else if (type === 'topic') data = await api.courses.topicContents(id)
      else if (type === 'material') data = await api.courses.materialContents(id)
      else if (type === 'folder') data = await api.courses.folderContents(id)
      else data = null
    ...
    try {
      ...
      if (editTarget) {
        ...
        if (editTarget.kind === 'semester') {
          ...
          await api.courses.updateSemester(editTarget.id, { title: form.title, delay_published_at: delayIso })
        } else if (editTarget.kind === 'subject') {
          await api.courses.updateSubject(editTarget.id, { title: form.title, description: form.description || '' })
        } else if (editTarget.kind === 'topic') {
          await api.courses.updateTopic(editTarget.id, { title: form.title, description: form.description || '' })
          ...
          await api.courses.updateMaterial(editTarget.id, payload)
          ...
          await api.courses.updateFolder(editTarget.id, { title: form.title })
        } else if (editTarget.kind === 'file') {
          await api.courses.updateFile(editTarget.id, { title: form.title || '', file_role: form.file_role })
        }
      ...
      if (createKind === 'semester') {
        ...
        await api.courses.createSemester({ course: cur.id, title: form.title, delay_published_at: delayIso })
        ...
        if (nodeType === 'course') await api.courses.createMaterial({ course: cur.id, ...common, extra })
        else if (nodeType === 'topic') await api.courses.createMaterial({ topic: cur.id, ...common, extra })
        else {
        ...
        if (nodeType === 'material') {
          await api.courses.createFolder({ material: cur.id, parent: null, title: form.title })
        } else if (nodeType === 'folder') {
          await api.courses.createFolder({ material: cur.material, parent: cur.id, title: form.title })
        } else {
  ...
  const deleteItem = async (kind, id) => {
    ...
    setError('')
    try {
      if (kind === 'course') await api.courses.deleteCourse(id)
      else if (kind === 'semester') await api.courses.deleteSemester(id)
      else if (kind === 'subject') await api.courses.deleteSubject(id)
      else if (kind === 'topic') await api.courses.deleteTopic(id)
      else if (kind === 'assignment') await api.courses.assignments.remove(id)
      else if (kind === 'material') await api.courses.deleteMaterial(id)
      else if (kind === 'folder') await api.courses.deleteFolder(id)
      else if (kind === 'file') await api.courses.deleteFile(id)
