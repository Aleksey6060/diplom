...
export default function CourseOverviewPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        const list = await api.courses.tests.results(Number(currentTestId))
        if (cancelled) return
    ...
    return () => { cancelled = true }
  }, [currentMaterialType, currentNodeType, currentTestAttemptsLimit, currentTestId, testAttemptsVersion])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setAssignmentsError('')
      try {
        const list = await api.courses.assignments.myByTopic(nodeId)
        if (cancelled) return
    ...
    return () => { cancelled = true }
  }, [currentNodeType, isLoading, nodeId])
    ...
    <div className="pb-20 space-y-4">
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          {!items.length && !error && (
            <div className="w-full max-w-7xl mx-auto px-4" style={{ color: 'var(--content-text-muted)' }}>Пусто</div>
          )}
        </div>
      )}
