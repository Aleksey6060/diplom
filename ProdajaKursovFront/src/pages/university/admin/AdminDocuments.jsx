...
export default function AdminDocuments() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setError('')
      try {
        const list = await api.documents.listAll({ universitySlug })
  ...
  const onPickFiles = async (e) => {
    ...
    try {
      for (const f of files) {
        ...
        try {
          const saved = await api.documents.upload(f, { universitySlug })
  ...
  const remove = async (id) => {
    ...
    setLoading(true)
    try {
      await api.documents.remove(id, { universitySlug })
