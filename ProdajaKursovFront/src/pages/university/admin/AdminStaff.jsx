...
export default function AdminStaff() {
    ...
    ;(async () => {
      try {
        const [r, s, p] = await Promise.all([
          canAccessRolesList ? api.users.roles.list(uniOpts) : Promise.resolve([]),
          canViewStaff ? api.users.staff.list(uniOpts) : Promise.resolve([]),
          (canCreateRole || canConfigureRole)
            ? api.users.permissions.modules(uniOpts).catch(() => [])
            : Promise.resolve([]),
        ])
        if (!cancelled) {
  ...
  }, [accessModules, universitySlug, canAccessRolesList, canViewStaff, canCreateRole, canConfigureRole])
  ...
  const submitNewStaff = async () => {
    ...
    setStaffCreateError('')
    if (!form.lastName || !form.firstName || !form.email || !form.accountType || !form.password) {
      setStaffCreateError('Заполните обязательные поля')
      return
    }
    if (form.accountType === 'employee' && !form.roleId) {
      setStaffCreateError('Выберите роль')
      return
    }
    try {
      ...
      setStaffCreateError(formatApiError(e, 'Не удалось создать сотрудника'))
    }
  }
  ...
  const submitEditStaff = async () => {
    ...
    setStaffEditError('')
    if (!editForm.lastName || !editForm.firstName || !editForm.email || !editForm.accountType) {
      setStaffEditError('Заполните обязательные поля')
      return
    }
    if (editForm.accountType === 'employee' && !editForm.roleId) {
      setStaffEditError('Выберите роль')
      return
    }
    const payload = {
    ...
    try {
      await api.users.staff.update(editForm.id, payload, uniOpts)
      const [r, s] = await Promise.all([
        canAccessRolesList ? api.users.roles.list(uniOpts) : Promise.resolve([]),
        canViewStaff ? api.users.staff.list(uniOpts) : Promise.resolve([]),
      ])
      ...
      setStaffEditError(formatApiError(e, 'Не удалось сохранить сотрудника'))
    }
  }
  ...
  const deleteStaff = async (id) => {
    ...
    setStaffEditError('')
    try {
      await api.users.staff.remove(id, uniOpts)
      if (canViewStaff) {
        const s = await api.users.staff.list(uniOpts)
        setItems(Array.isArray(s) ? s : [])
      } else {
        setItems([])
      }
      ...
      setStaffEditError(formatApiError(e, 'Не удалось удалить сотрудника'))
    }
  }
  ...
  const submitRole = async () => {
    ...
    try {
      await api.users.roles.create({ name, permission_codes: perms, is_active: true }, uniOpts)
      if (canAccessRolesList) {
        const r = await api.users.roles.list(uniOpts)
        setRoles(Array.isArray(r) ? r : [])
      } else {
        setRoles([])
      }
  ...
  const openRoleEdit = async (r) => {
    ...
    try {
      const full = await api.users.roles.retrieve(r.id, uniOpts)
      setRoleEditForm({
        id: full?.id,
        name: full?.name || '',
    ...
    <div className="space-y-6 pt-4">
      ...
      {showCreate && (
        <motion.div className="fixed inset-0 z-[9990] flex items-center justify-center">
          ...
          <motion.div initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="admin-card rounded-2xl w-full max-w-2xl p-6 relative">
      ...
      {showRolesList && (
        <motion.div className="fixed inset-0 z-[9990] flex items-center justify-center">
          ...
          <motion.div initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="modal-panel rounded-2xl border border-white/15 w-full max-w-2xl p-6 relative h-[81vh] overflow-hidden">
            <div className="h-full flex flex-col">
              ...
              <div className="space-y-3 flex-1 overflow-auto pr-1 custom-scrollbar">
                {roles.map(r => (
                  <div key={r.id} className="bg-white/50 rounded-2xl border border-black/5 p-5 space-y-3 transition-all hover:bg-white/70">
                    <div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 w-full">
                        ...
                        <button
                          onClick={async () => {
                            ...
                            setRoleStaffError('')
                            try {
                              const list = await api.users.roles.staffs(r.id, uniOpts)
                              setRoleStaffRole(r)
                        ...
                        <button
                          onClick={async () => {
                            ...
                            await api.users.roles.remove(r.id, uniOpts)
                            const next = await api.users.roles.list(uniOpts)
                ...
                <div key={u.id} className="bg-white/50 rounded-2xl border border-black/5 p-4 flex items-center justify-between gap-4">
                  ...
                  <button
                    onClick={async () => {
                      setRoleStaffError('')
                      try {
                        await api.users.staff.update(u.id, { role: null }, uniOpts)
