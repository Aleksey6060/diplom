...
export default function AdminBanner() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setError('')
      try {
        const data = await api.banner.manage({ universitySlug })
  ...
  const onPick = async (file) => {
    ...
    setLoading(true)
    try {
      const saved = await api.banner.upload(file, { is_active: true, universitySlug })
        ...
        <div className="mt-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <button
            ...
            onClick={async () => {
              ...
              setLoading(true)
              try {
                const saved = await api.banner.toggle(!enabled, { universitySlug })
                setBanner(saved && typeof saved === 'object' ? saved : null)
              } catch (e) {
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <button
                ...
                onClick={async () => {
                  ...
                  setLoading(true)
                  try {
                    await api.banner.remove({ universitySlug })
                    setBanner(null)
                  } catch (e) {
