...
export default function AdminStudents() {
  ...
  const openGroup = async (g) => {
    ...
    setLoading(true)
    try {
      const list = await api.users.groups.courses(g.id, { universitySlug })
  ...
  const openCourse = async (c) => {
    ...
    setLoading(true)
    try {
      const contents = await api.courses.contents(c.id, { universitySlug })
  ...
  const openSemester = async (s) => {
    ...
    setLoading(true)
    try {
      const list = await api.users.groups.participants(selectedGroup.id, { universitySlug })
  ...
  const openScheduleBuilder = async (s) => {
    ...
    setScheduleCells({})
    try {
      const data = await api.users.groups.schedule.get(selectedGroup.id, selectedCourse.id, s.id, { universitySlug })
  ...
  const removeScheduleBinding = async (bindingId) => {
    ...
    setScheduleError('')
    try {
      await api.users.groups.teacherAttachments.remove(selectedGroup.id, selectedCourse.id, bindingId, { universitySlug })
  ...
  const saveSchedule = async () => {
    ...
    try {
      ...
      await api.users.groups.schedule.save(selectedGroup.id, selectedCourse.id, scheduleSemester.id, payload, { universitySlug })
  ...
  const commitGrade = async ({ studentId, assignmentId, valueRaw, currentValue = null }) => {
    ...
    try {
      if (normalizedCurrent == null) {
        await api.courses.grades.set({ assignment: Number(assignmentId), student: Number(studentId), value, comment: '' }, { universitySlug })
      } else {
        await api.courses.grades.update({ assignment: Number(assignmentId), student: Number(studentId), value, comment: '' }, { universitySlug })
      }
      setSubjectPerf(prev => {
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const [asgs, tests] = await Promise.all([
                            api.courses.assignments.list(s.id, { universitySlug }),
                            api.courses.tests.bySubject(s.id, { universitySlug }),
                          ])
                    ...
                    <div className="space-y-2 flex-1 min-h-0 overflow-auto pr-1 custom-scrollbar">
                      {(Array.isArray(scheduleBindings) ? scheduleBindings : []).map(b => (
                        <div
                          ...
                          className={`rounded-xl border bg-white/5 px-3 py-2 cursor-grab active:cursor-grabbing flex items-start justify-between gap-3 ${String(scheduleMobileBindingId) === String(b?.id ?? '') ? 'border-emerald-400 bg-emerald-500/10' : 'border-emerald-500/40'}`}
                        >
                          <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 overflow-auto pb-2 -mx-1 px-1">
                          {[0, 1, 2, 3, 4, 5].map(w => (
                            <button
                              ...
                              type="button"
                              onClick={() => setScheduleMobileWeekday(w)}
                              className={`px-3 py-2 rounded-xl border whitespace-nowrap ${scheduleMobileWeekday === w ? '!bg-emerald-600 !border-emerald-500/20' : '!bg-white/5 !border-white/10'}`}
                              style={scheduleMobileWeekday === w ? { color: '#fff' } : { color: contentText }}
                            >
                              {weekdayLabels[w]}
                            </button>
                          ))}
                                ...
                                <div className="flex items-center justify-between gap-3 mb-2">
                                  ...
                                  {cell?.subjectId != null && (
                                    <button
                                      type="button"
                                      onClick={() => clearCell(cellKey)}
                                      className="w-9 h-9 rounded-xl !bg-white/10 !border-white/10 border text-white/80 flex items-center justify-center"
                                    >
                                      ×
                                    </button>
                                  )}
                                ...
                                >
                                  {cell?.subjectId != null ? (
                                    <div
                                      ...
                                      className="rounded-2xl bg-white/10 border border-white/10 px-3 py-2 cursor-grab active:cursor-grabbing"
                                    >
                                      <div className="text-white text-sm font-semibold break-words">{subjectTitle}</div>
                                      <div className="text-[#266479] text-xs mt-1 break-words">{teacherName}</div>
                                    </div>
                                  ) : (
                                    <div className="text-white/40 text-sm">{picked ? 'Нажмите, чтобы поставить выбранный предмет' : 'Выберите предмет выше'}</div>
                                  )}
                                </div>
                              </div>
                            )
