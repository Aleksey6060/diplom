...
function BulkUploadButton({ onUploaded }) {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      await api.users.students.manyCreate(normalized, { ignore_conflicts: true, universitySlug })
...
export default function AdminDistribution() {
  ...
  const [isParticipantsLoading, setParticipantsLoading] = useState(false)
  const normalizeList = (value) => (Array.isArray(value) ? value : (Array.isArray(value?.results) ? value.results : []))
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      try {
        const list = await api.users.students.list({ universitySlug })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const list = await api.users.groups.list({ universitySlug })
  ...
  useEffect(() => {
    let cancelled = false
    if (!filterGroupId || filterGroupId === NO_GROUP_ID) {
    ...
    ;(async () => {
      ...
      try {
        const list = await api.users.groups.participants(filterGroupId, { universitySlug })
        if (!cancelled) setGroupParticipants(normalizeList(list))
      } catch {
  ...
  const reloadParticipants = async () => {
    if (!filterGroupId || filterGroupId === NO_GROUP_ID) return
    const list = await api.users.groups.participants(filterGroupId, { universitySlug })
    setGroupParticipants(normalizeList(list))
  }
  const confirmAttach = async () => {
    ...
    setGroupActionError('')
    try {
      await api.users.groups.addToGroup(attachGroupId, attachables, { universitySlug })
      await Promise.all([
        reloadStudents(),
        reloadGroups(),
        attachGroupId && filterGroupId && String(attachGroupId) === String(filterGroupId) ? reloadParticipants() : Promise.resolve(),
      ])
      setSelectedIds([])
    } catch (e) {
      const body = e?.body
  ...
  const detachFromGroup = async (gid, uid) => {
    setGroupActionError('')
    try {
      await api.users.groups.removeFromGroup(gid, [uid], { universitySlug })
      await Promise.all([
        reloadStudents(),
        reloadGroups(),
        filterGroupId ? reloadParticipants() : Promise.resolve(),
      ])
    } catch (e) {
  ...
  const submitCreateGroup = async () => {
    ...
    setCreateGroupError('')
    try {
      await api.users.groups.create({ name: groupForm.name.trim() }, { universitySlug })
      await reloadGroups()
      setGroupForm({ name: '' })
  ...
  const submitRenameGroup = async () => {
    ...
    try {
      ...
      await api.users.groups.update(renameGroupId, { name }, { universitySlug })
      await reloadGroups()
  ...
  const submitDeleteGroup = async () => {
    ...
    try {
      ...
      await api.users.groups.remove(deleteGroupId, { universitySlug })
  ...
  const loadAllSimpleCourses = async () => {
    ...
    try {
      const list = await api.courses.list({ universitySlug: universitySlug })
  ...
  const saveStudentCourses = async () => {
    ...
    setStudentCoursesLoading(true)
    try {
      await api.users.students.bindToSimpleCourses(studentCoursesStudentId, selectedSimpleCourseIds, { universitySlug })
  ...
  const detachSimpleCourse = async (courseId) => {
    ...
    try {
      ...
      await api.users.students.bindToSimpleCourses(studentCoursesStudentId, newIds, { universitySlug })
  ...
  const loadGroupCourses = async (groupId) => {
    ...
    try {
      const list = await api.users.groups.courses(groupId, { universitySlug })
      const arr = normalizeList(list)
      setGroupCourses(arr)
  ...
  const loadAllCourses = async () => {
    ...
    setAllCoursesLoading(true)
    try {
      const list = await api.courses.list({ universitySlug })
  ...
  const saveGroupCourses = async () => {
    ...
    setGroupCoursesLoading(true)
    try {
      await api.users.groups.bindToCourses(groupCoursesGroupId, selectedCourseIds, { universitySlug })
  ...
  const detachCourse = async (courseId) => {
    ...
    setGroupCoursesError('')
    try {
      await api.users.groups.detachFromGroup(groupCoursesGroupId, courseId, { universitySlug })
  ...
  const loadTeacherAttachData = async () => {
    ...
    try {
      const [semestersRes, staffRes] = await Promise.all([
        api.users.groups.courseSemesters(groupCoursesGroupId, courseId, { universitySlug }),
        api.users.staff.list({ universitySlug }),
      ])
      ...
      const [attachmentsRes, subjectsRes] = await Promise.all([
        api.users.groups.teacherAttachments.list(groupCoursesGroupId, courseId, { semesterId: selectedSemesterId, universitySlug }),
        api.users.groups.semesterSubjects(groupCoursesGroupId, courseId, selectedSemesterId, { universitySlug }),
      ])
  ...
  const removeTeacherAttachment = async (attachmentId) => {
    ...
    setTeacherAttachLoading(true)
    try {
      await api.users.groups.teacherAttachments.remove(groupCoursesGroupId, courseId, attachmentId, { universitySlug })
      await loadTeacherAttachData()
    } catch (e) {
  ...
  const submitCreateStudent = async () => {
    ...
    try {
      ...
      await api.users.students.create(payload, { universitySlug })
      await Promise.all([reloadStudents(), reloadGroups(), reloadParticipants()])
      setForm(emptyStudentForm)
      setShowCreate(false)
    } catch (e) {
  ...
  const submitEdit = async () => {
    ...
    try {
      ...
      await api.users.students.update(id, payload, { universitySlug })
      await Promise.all([reloadStudents(), reloadGroups(), reloadParticipants()])
      setEditOpen(false)
    } catch (e) {
  ...
  const submitDeleteStudent = async () => {
    ...
    try {
      ...
      await api.users.students.remove(deleteStudentId, { universitySlug })
      await Promise.all([reloadStudents(), reloadGroups()])
      if (filterGroupId && filterGroupId !== NO_GROUP_ID) await reloadParticipants()
  ...
  const filtered = useMemo(() => {
    ...
    return filterGroupId ? groupParticipants : students
  }, [students, groupParticipants, filterGroupId, NO_GROUP_ID])
    ...
    <Motion.div className="space-y-6 pt-4" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 space-y-3">
          ...
          {filterGroupId && filterGroupId !== NO_GROUP_ID && isParticipantsLoading && (
              ...
              {filtered
                ...
                .map(s => (
                  <Motion.div
                    ...
                    className="admin-card rounded-2xl p-4 flex items-center justify-between"
                  >
                    ...
                    <div className="flex items-center gap-2 flex-wrap justify-end overflow-x-auto whitespace-nowrap flex-none">
                      ...
                      {filterGroupId && filterGroupId !== NO_GROUP_ID && (
