...
export default function TeacherGroupDetails() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const data = await api.groups.teacher.semesters(id)
        if (cancelled) return
  ...
  useEffect(() => {
    try {
      const key = `teacher_assignments_semester_${id}`
  ...
  const loadAssignments = useCallback(async (signal) => {
    ...
    try {
      ...
      const loader = api?.groups?.teacher?.groupAssignments
        ? () => api.groups.teacher.groupAssignments(id, { semesterId })
        : () => apiRequest(`/api/groups/teacher/groups/${id}/assignments/`, { method: 'GET', signal })
  ...
  const items = useMemo(() => {
    const base = (Array.isArray(assignments) ? assignments : []).filter(a => {
      ...
      <div className="space-y-3">
        {items.map(it => (
          <div
            ...
            role="button"
            tabIndex={0}
            onClick={() => navigate(`/teacher/groups/${id}/assignments/${it.id}`)}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') navigate(`/teacher/groups/${id}/assignments/${it.id}`) }}
